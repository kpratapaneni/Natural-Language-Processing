...
class IMDBReader:
    ...
    def get_texts(self, subset):
        ...
        for textfile in os.listdir(os.path.join(self.data_dir, subset)):
            if textfile[-4:] == ".txt":
                with open(os.path.join(self.data_dir, subset, textfile)) as f:
            ...
            for text in data.get_texts(classval):
                ...
                for word in text.get_words():
                    if word in self.vocab:
                        sum_positive += log((self.count_positive[word] + alpha) / ( self.total_positive_words + (len(self.vocab) * alpha))
                        )
                        sum_negative += log((self.count_negative[word] + alpha) / ( self.total_negative_words + (len(self.vocab) * alpha)))
