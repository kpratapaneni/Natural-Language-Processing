...
export BERT_BASE_DIR=$HOME/NLP585/hw4/bert
...
cat data/lang_id_eval.csv | cut -d ',' -f 2- | awk 'NR>1 {print}' >> $INPUT_DIR/eval.txt

cat data/lang_id_test.csv | cut -d ',' -f 2- | awk 'NR>1 {print}' >> $INPUT_DIR/test.txt

cat data/lang_id_train.csv | cut -d ',' -f 2- | awk 'NR>1 {print}' >> $INPUT_DIR/train.txt
